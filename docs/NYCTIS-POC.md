...
(`rust/Cargo.toml`: `path = "../../nyctis/crates/nyctis-codec"` and the other two), so this checkout expects the protocol repository at `../nyctis`; alone it
directory is not a standalone checkout: cloning it alone gives a tree that does not build. There
is no `nyctis-tx` dependency; nothing here builds a transition yet.
...
`nyctis_state::State::apply` uses `msg_id` as its replay-protection key and folds it into
...
same seed it already holds, so the Nyctis identity comes along with the wallet with
no extra backup:

```
ask  = Fr(BLAKE2b-512("nyctis.key.v0.ask." ‖ "spend" ‖ seed))
...
`NyView.vk_hash` — the hash of the key the replay *actually* verified against.
...
The hash the wallet pins with is on the result: `NyView.vk_hash`, lowercase hex of the
...
zatoshi, the reference CLI's default — which is the one cost of a Nyctis payment denominated in
