/// Turning the notes a Nightjar replay reports into the *messages* that moved
/// them, which is the only form a history can be read out of.
///
/// `NjView.notes` is every note this wallet has ever owned on the channel,
...
/// (the contract is the doc comment on `nightjarReplay` in
/// `lib/src/rust/api/nightjar.dart`):
///
...
///   its own kind ([NightjarActivityKind.selfTransfer]), the *gross* amount
...
///   signal, and [NightjarActivityKind.settling] is it on screen — without a
...
/// ([NightjarActivityItem.blockLabel]) so the ordering stays legible when the
/// timestamps are not.
///
...
/// What one Nightjar message did to this wallet.
enum NightjarActivityKind {
...
/// One row's worth of Nightjar history: what one message did to this wallet's
...
@immutable
class NightjarActivityItem {
  const NightjarActivityItem({
    required this.msgId,
  ...
  const NightjarActivityItem.settling({
    required int count,
    ...
    this.timestamp,
       ...
       assetId = '',
       kind = NightjarActivityKind.settling,
       delta = null,
       moved = null,
       decimals = 0,
       height = viewHeight,
  ...
  /// [NightjarActivityKind.settling], which is about no message in particular.
  final String msgId;
  ...
  final NightjarActivityKind kind;
  ...
  /// [NightjarActivityKind.settling] row. Zero on every other row.
  ...
  bool get isIssuance =>
      totalInputs == 0 && kind == NightjarActivityKind.received;

  /// The asset as a row names it: the declared name, or the truncated asset id
  /// when the issuer never published one — the same choice the Nightjar asset
  /// rows make, so the two surfaces show the same string.
  String get assetLabel =>
      hasName ? name!.trim() : truncateNightjarAssetId(assetId);
  ...
  String get stableId => switch (kind) {
    NightjarActivityKind.settling => 'nightjar-settling',
    _ => 'nightjar-msg:$msgId:$assetId',
  };
}

/// Every Nightjar message that touched this wallet, newest block first.
///
/// [blockTimes] is keyed by block height; a height it does not carry yields a
/// null [NightjarActivityItem.timestamp] rather than a guess. The order is
...
/// pins it.
List<NightjarActivityItem> buildNightjarActivityItems({
  ...
  final items = <NightjarActivityItem>[];
  for (final message in messages) {
    items.addAll(message.toItems(blockTimes));
  }
  items.sort(compareNightjarActivityItems);

  // The ten-block window, and only where the wallet has Nightjar history of
...
int compareNightjarActivityItems(
  NightjarActivityItem a,
  NightjarActivityItem b,
) {
  final aSettling = a.kind == NightjarActivityKind.settling;
  final bSettling = b.kind == NightjarActivityKind.settling;
...
List<int> nightjarActivityMessageHeights(NightjarViewData view) {
  final heights = <int>{};
...
NightjarActivityItem? _settlingItem(NightjarViewData view, DateTime? at) {
  if (view.pendingMessageCount <= 0) return null;
  if (view.isUnverified || !view.isConfigured) return null;
  return NightjarActivityItem.settling(
...
class _OwnedNote {
  ...
  final int decimals;
  final BigInt amount;
  final NightjarNoteRowData note;
}
...
class _Message {
  ...
  List<NightjarActivityItem> toItems(Map<int, DateTime> blockTimes) {
  ...
  NightjarActivityKind _kindFor({
    ...
    if (inSum == outSum) return NightjarActivityKind.selfTransfer;
    return NightjarActivityKind.sent;
  }
...
List<_Message> _groupByMessage(NightjarViewData view) {
