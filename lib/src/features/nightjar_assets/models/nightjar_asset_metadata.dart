...
const String kNightjarMetadataSchema = 'nightjar-asset-metadata/1';

/// The family half of that string, and the major this revision implements.
/// Split out so that [nightjarSchemaMajor] can serve this document and the
/// collection document of `spec/asset-collection-v0.md` from one rule.
const String kNightjarMetadataSchemaFamily = 'nightjar-asset-metadata';
const int kNightjarMetadataSchemaMajor = 1;

/// Section 4.1: `description` is truncated here, never rendered as rich text
/// and never rendered as a link.
const int kNightjarMetadataMaxDescriptionChars = 1000;

/// Section 4.3: at most 16 links.
const int kNightjarMetadataMaxLinks = 16;
...
const Set<String> kNightjarKnownLinkRels = {
  'x',
  'github',
  'discord',
  'telegram',
...
class NightjarMetadataFormatException implements Exception {
  const NightjarMetadataFormatException(this.message);

  /// Diagnostic, not user-facing.
  final String message;

  @override
  String toString() => 'NightjarMetadataFormatException: $message';
}

/// The `logo` member (section 4.2).
class NightjarAssetLogoRef {
  const NightjarAssetLogoRef({required this.uri, this.digestB2});
...
class NightjarAssetLink {
  const NightjarAssetLink({required this.rel, required this.uri});
  ...
  /// ignore the rest.
  bool get isRecognized => kNightjarKnownLinkRels.contains(rel);
}

/// A parsed metadata document.
class NightjarAssetMetadata {
  const NightjarAssetMetadata({
    this.description,
    this.logo,
    this.website,
    this.links = const [],
  ...
  /// Plain text, already truncated to [kNightjarMetadataMaxDescriptionChars]
  ...
  /// [kNightjarMetadataMaxLinks]. Filter with [renderableLinks] to draw.
  ...
  factory NightjarAssetMetadata.parseBytes(List<int> bytes) =>
      NightjarAssetMetadata.fromJson(nightjarDecodeDocument(bytes));

  factory NightjarAssetMetadata.parse(String source) {
    final Object? decoded;
    try {
      decoded = jsonDecode(source);
    } on FormatException catch (error) {
      throw NightjarMetadataFormatException('not JSON: ${error.message}');
    }
    if (decoded is! Map<String, Object?>) {
      throw const NightjarMetadataFormatException(
        'top level is not a JSON object',
      );
    }
    return NightjarAssetMetadata.fromJson(decoded);
  }

  factory NightjarAssetMetadata.fromJson(Map<String, Object?> json) {
    final major = nightjarSchemaMajor(
      json['schema'],
      kNightjarMetadataSchemaFamily,
    );
    if (major != kNightjarMetadataSchemaMajor) {
  ...
  static String? _description(Object? value) {
    ...
    if (runes.length <= kNightjarMetadataMaxDescriptionChars) return cleaned;
    return String.fromCharCodes(
      runes.take(kNightjarMetadataMaxDescriptionChars),
    );
  }

  static NightjarAssetLogoRef? _logo(Object? value) =>
      nightjarAssetLogoRef(value);
}
...
/// the format is decided from the bytes by [sniffNightjarImageFormat] with the
/// SVG refusal in it.
///
...
NightjarAssetLogoRef? nightjarAssetLogoRef(Object? value) {
  if (value is! Map<String, Object?>) return null;
  final uri = nightjarHttpsUri(value['uri']);
  if (uri == null) return null;
  final digest = value['b2'];
  return NightjarAssetLogoRef(
    uri: uri,
    digestB2: digest is String && isNightjarB2Digest(digest) ? digest : null,
  );
}
...
List<NightjarAssetLink> nightjarMetadataLinks(Object? value) {
  if (value is! List) return const [];
  final links = <NightjarAssetLink>[];
  for (final entry in value) {
    if (links.length >= kNightjarMetadataMaxLinks) break;
    ...
    final uri = nightjarHttpsUri(entry['uri']);
    if (uri == null) continue;
    links.add(NightjarAssetLink(rel: token, uri: uri));
  }
...
Uri? nightjarHttpsUri(Object? value) {
  if (value is! String) return null;
...
int? nightjarSchemaMajor(Object? value, String family) {
  if (value is! String) return null;
  final prefix = '$family/';
...
/// [NightjarMetadataFormatException].
///
...
Map<String, Object?> nightjarDecodeDocument(List<int> bytes) {
  if (bytes.length >= 3 &&
      bytes[0] == 0xEF &&
      bytes[1] == 0xBB &&
      bytes[2] == 0xBF) {
    throw const NightjarMetadataFormatException('document starts with a BOM');
  }
  final String text;
  try {
    text = utf8.decode(bytes);
  } on FormatException catch (error) {
    throw NightjarMetadataFormatException('not UTF-8: ${error.message}');
  }
  final Object? decoded;
  try {
...
bool isNightjarB2Digest(String value) =>
...
String sanitizeNightjarMetadataText(String input) {
...
String nightjarUriOrigin(Uri uri) {
  final host = uri.host;
