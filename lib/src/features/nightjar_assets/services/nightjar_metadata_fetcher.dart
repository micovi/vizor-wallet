...
/// other part of Nightjar is built so that holders are invisible; a wallet
...
/// * every caller in this feature is behind [NightjarAssetAcceptance], and
...
import '../models/nightjar_collection_metadata.dart';
import '../models/nightjar_metadata_pointer.dart';
import 'nightjar_image_format.dart';
import 'nightjar_metadata_digest.dart';
import 'nightjar_metadata_reader.dart';
import 'nightjar_metadata_transport.dart';

export 'nightjar_metadata_reader.dart'
    show
        NightjarMetadataAbandonReason,
        kNightjarAssetTotalMaxBytes,
        kNightjarDocumentMaxBytes,
        kNightjarLogoMaxBytes,
        kNightjarMetadataMaxRedirects,
        kNightjarMetadataTimeout;
...
class NightjarAssetMetadataView {
  const NightjarAssetMetadataView({
  ...
  final NightjarAssetMetadata metadata;
...
/// fetching it once.
class NightjarCollectionMemberView {
  const NightjarCollectionMemberView({
    required this.assetId,
    required this.index,
  ...
  final NightjarCollectionMetadata collection;

  /// The template resolved for [index].
  final NightjarCollectionMember member;
  ...
  final NightjarMetadataAbandonReason? imageReason;
...
class NightjarMetadataFetchOutcome {
  const NightjarMetadataFetchOutcome.fetched(
    NightjarAssetMetadataView this.view,
  ) : reason = null;

  const NightjarMetadataFetchOutcome.abandoned(
...
class NightjarArtworkFetchOutcome {
  const NightjarArtworkFetchOutcome.asset(
    NightjarAssetMetadataView this.assetView,
  ) : memberView = null,
      reason = null;

  const NightjarArtworkFetchOutcome.collectionMember(
    NightjarCollectionMemberView this.memberView,
  ) : assetView = null,
      reason = null;

  const NightjarArtworkFetchOutcome.abandoned(
    NightjarMetadataAbandonReason this.reason,
  ) : assetView = null,
      memberView = null;

  /// Set when the document was an `asset-metadata-v0.md` one.
  final NightjarAssetMetadataView? assetView;

  /// Set when it was an `asset-collection-v0.md` one and an index was given.
  final NightjarCollectionMemberView? memberView;
...
class NightjarAssetMetadataFetcher {
  NightjarAssetMetadataFetcher({
    NightjarMetadataTransport? transport,
    int maxRedirects = kNightjarMetadataMaxRedirects,
    Duration timeout = kNightjarMetadataTimeout,
  }) : _reader = NightjarMetadataReader(
         transport: transport,
         maxRedirects: maxRedirects,
         timeout: timeout,
       );

  final NightjarMetadataReader _reader;
  ...
  final Map<String, NightjarArtworkFetchOutcome> _cache = {};

  /// In-flight per-asset fetches, so two widgets that appear in the same frame
  /// make one request rather than two.
  final Map<String, Future<NightjarArtworkFetchOutcome>> _inFlight = {};
  ...
  final Map<String, NightjarCollectionMetadata> _collectionDocuments = {};

  final Map<String, Future<NightjarCollectionMetadata>>
  ...
  /// triggers a fetch.
  NightjarMetadataFetchOutcome? cached({
  ...
  NightjarArtworkFetchOutcome? cachedArtwork({
    required String assetId,
    required String? uri,
  }) {
    final pointer = readNightjarMetadataUri(uri).pointer;
  ...
  /// [NightjarMetadataAbandonReason.documentRefused], because this entry point
  ...
  Future<NightjarMetadataFetchOutcome> fetch({
    required String assetId,
    required String? uri,
  }) async {
  ...
  Future<NightjarArtworkFetchOutcome> fetchArtwork({
    required String assetId,
    required String? uri,
    int? index,
  }) {
    ...
    if (pointer == null) {
      return Future.value(
        const NightjarArtworkFetchOutcome.abandoned(
          NightjarMetadataAbandonReason.pointerRefused,
        ),
      );
    }
  ...
  static String _cacheKey(String assetId, NightjarMetadataPointer pointer) =>
  ...
  static String _documentKey(NightjarMetadataPointer pointer) =>
      pointer.digestB2 ?? '${pointer.requestUri}';

  Future<NightjarArtworkFetchOutcome> _fetch(
    String assetId,
    NightjarMetadataPointer pointer,
    int? index,
  ) async {
    final budget = NightjarResourceBudget();
    final document = await _document(assetId, pointer, budget);

    final reason = document.reason;
    if (reason != null) {
      return NightjarArtworkFetchOutcome.abandoned(reason);
    }
    ...
    return NightjarArtworkFetchOutcome.asset(
  ...
  Future<NightjarArtworkFetchOutcome> _collectionMember({
    required String assetId,
    required NightjarMetadataPointer pointer,
    required NightjarCollectionMetadata collection,
    required int? index,
    ...
    if (index == null) {
      ...
      return const NightjarArtworkFetchOutcome.abandoned(
        NightjarMetadataAbandonReason.documentRefused,
      );
    }
    ...
    if (member == null || !member.hasImage) {
      ...
      return NightjarArtworkFetchOutcome.collectionMember(
        NightjarCollectionMemberView(
          assetId: assetId,
          index: index,
          collection: resolved,
          member:
              member ??
              NightjarCollectionMember(
                index: index,
                name: null,
                description: null,
                image: null,
                digestB2: null,
                imageRejection: NightjarItemImageRejection.absent,
                attributes: const [],
              ),
          documentPinned: pointer.isPinned,
          sourceOrigin: pointer.origin,
          imageReason: NightjarMetadataAbandonReason.noImage,
  ...
  Future<_Image> _collectionLogo(
    NightjarMetadataPointer pointer,
    NightjarCollectionMetadata collection,
  ) {
  ...
  /// must not be one.
  Future<NightjarCollectionMetadata> _collectionDocument(
    NightjarMetadataPointer pointer,
    NightjarCollectionMetadata collection,
  ) {
    ...
    final future = Future(() async {
      ...
      if (table != null) {
        ...
        value = (bytes == null ? null : value.resolveDigestTable(bytes)) ?? value;
      }
      ...
      if (itemsDoc != null) {
        ...
        if (itemsDoc.bytes <= kNightjarItemsDocumentMaxBytes) {
          final result = await _reader.read(
            itemsDoc.uri,
  ...
  Future<_Document> _readDocument(
    ...
    final read = await _reader.read(
      pointer.requestUri,
      maxBytes: kNightjarDocumentMaxBytes,
      accept: 'application/json',
      budget: budget,
    );
    ...
    if (expected != null && !nightjarB2DigestMatches(expected, bytes)) {
    ...
    if (_declaresCollectionSchema(bytes)) {
      try {
        return _Document.collection(
          NightjarCollectionMetadata.parseBytes(bytes),
        );
      } on NightjarMetadataFormatException {
        return const _Document.abandoned(
          NightjarMetadataAbandonReason.documentRefused,
        );
      }
    ...
    try {
      return _Document.asset(NightjarAssetMetadata.parseBytes(bytes));
    } on NightjarMetadataFormatException {
      return const _Document.abandoned(
        NightjarMetadataAbandonReason.documentRefused,
      );
  ...
  static bool _declaresCollectionSchema(Uint8List bytes) {
    ...
    try {
      decoded = nightjarDecodeDocument(bytes);
    } on NightjarMetadataFormatException {
      return false;
    }
    return nightjarSchemaMajor(
          decoded['schema'],
  ...
  Future<_Image> _image({
    ...
    if (uri == null) {
      return const _Image.abandoned(NightjarMetadataAbandonReason.noImage);
    }
    final result = await _reader.read(
      uri,
      maxBytes: kNightjarLogoMaxBytes,
      accept: _imageAccept,
      budget: budget,
    );
    final bytes = result.bytes;
    ...
    if (expectedDigest != null &&
        !nightjarB2DigestMatches(expectedDigest, bytes)) {
    ...
    if (!sniffNightjarImageFormat(bytes).isRenderable) {
      return const _Image.abandoned(
  ...
  /// collections. A collection member has no [NightjarAssetMetadata] to hand
  /// back, so it reads as a refused document — which is what it is to a caller
  /// that has no index.
  static NightjarMetadataFetchOutcome _asAssetOutcome(
    NightjarArtworkFetchOutcome outcome,
  ) {
    final view = outcome.assetView;
    if (view != null) return NightjarMetadataFetchOutcome.fetched(view);
    return NightjarMetadataFetchOutcome.abandoned(
      outcome.reason ?? NightjarMetadataAbandonReason.documentRefused,
    );
  }
}
...
class _Document {
  const _Document.asset(NightjarAssetMetadata this.asset)
    : collection = null,
      reason = null;

  const _Document.collection(NightjarCollectionMetadata this.collection)
    : asset = null,
      reason = null;

  const _Document.abandoned(NightjarMetadataAbandonReason this.reason)
    : asset = null,
      collection = null;

  final NightjarAssetMetadata? asset;
  final NightjarCollectionMetadata? collection;
  final NightjarMetadataAbandonReason? reason;
}
...
class _Image {
  ...
  const _Image.abandoned(NightjarMetadataAbandonReason this.reason)
    : bytes = null;
