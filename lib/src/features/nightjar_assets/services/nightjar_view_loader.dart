/// The real Nightjar view loader: indexer for data availability, Rust for
...
/// hands them to `nightjarReplay`, which verifies each Groth16 proof, replays
...
///   its own server advertises is not used at all.
...
/// Hands over the active account's seed bytes, or null when there are none to
/// hand over (a locked wallet, a hardware account).
///
/// A callback rather than a value so the loader controls *when* the seed is in
/// memory: it is read after every byte of network I/O is done, used, and
/// zeroed before either result is awaited — the discipline `send_flow.dart`
/// follows for signing.
typedef NightjarSeedReader = Future<Uint8List?> Function();
...
/// zero the seed between issuing a call and collecting its result. A test
/// subclasses this to drive the loader without the native library.
class NightjarReplayBridge {
  const NightjarReplayBridge();

  Future<String> channelId({
    required String network,
    required String channelUivk,
  }) => rust_nightjar.nightjarChannelId(
    network: network,
  ...
  Future<rust_nightjar.NjIdentity> identity({
    ...
    required String network,
  }) => rust_nightjar.nightjarIdentity(seed: seed, network: network);
...
///
/// The default because most callers of [loadNightjarView] are tests and
...
/// assuming them. [loadNightjarViewFor] supplies the real pair.
const _noZecSources = rust_nightjar.NjZecSources(
  dbPath: '',
  lightwalletdUrl: '',
);

/// Builds the view for `config` using `client`, the wallet's own chain tip,
/// and whatever `readSeed` hands over.
///
/// Separate from the provider so a test can drive it with a fake client, a
/// fake bridge and a known seed without standing up Riverpod, secure storage
/// or the native library.
Future<NightjarViewData> loadNightjarView({
  ...
  rust_nightjar.NjZecSources zecSources = _noZecSources,
  NightjarReplayBridge bridge = const NightjarReplayBridge(),
}) async {
  if (!config.isUsable) {
  ...
  /// The wallet's own Nightjar address, which depends on the seed and the
  /// network and on nothing the indexer said. Derived even on the failure
  /// paths so a misconfigured channel does not also take away the receive
  /// address.
  Future<NightjarIdentityData?> identityOnly() async {
    try {
      final seed = await readSeed();
      if (seed == null || seed.isEmpty) return null;
      final Future<rust_nightjar.NjIdentity> call;
      try {
        call = bridge.identity(seed: seed, network: config.networkName);
      } finally {
        seed.fillRange(0, seed.length, 0);
      }
      return _identityOf(await call, config);
    } catch (_) {
  ...
  Future<NightjarViewData> failure(
    NightjarViewStatus status, {
  ...
  try {
    ...
    return failure(
      NightjarViewStatus.unreachable,
      message: kNightjarUnreachableText,
  ...
  try {
    ...
    return failure(
      ...
      message:
          'This wallet cannot read the configured Nightjar channel key. '
          'Check the channel in Nightjar settings.',
  ...
  if (channelId != status.info.channelId) {
    return failure(
      NightjarViewStatus.unverified,
      message: kNightjarChannelMismatchText,
      detail:
  ...
  if (!key.matchesVkHash(status.info.vkHash)) {
    return failure(
      NightjarViewStatus.unverified,
      message: kNightjarVerifyingKeyMismatchText,
      detail:
  ...
  final tipSource = walletTip > 0
      ? NightjarChainTipSource.wallet
      : NightjarChainTipSource.indexer;
  final chainTip = walletTip > 0 ? walletTip : status.live.tip;

  final seed = await readSeed();
  if (seed == null || seed.isEmpty) {
    return const NightjarViewData(
      status: NightjarViewStatus.notConfigured,
      statusDetail: 'The wallet is locked.',
    );
  }

  // Both calls are issued before the seed is zeroed and awaited after it, so
  // the plaintext seed is live for the synchronous argument encoding and not
  // for the verification that follows — which, for a channel of a few hundred
  // messages, is the overwhelming majority of the wall clock here.
  final Future<rust_nightjar.NjIdentity> identityCall;
  final Future<rust_nightjar.NjView> replayCall;
  try {
    identityCall = bridge.identity(seed: seed, network: config.networkName);
    replayCall = bridge.replay(
      network: config.networkName,
      channelUivk: config.channelUivk,
  ...
  // Awaited first so a replay failure cannot leave this future unhandled.
  NightjarIdentityData? identity;
  try {
    identity = _identityOf(await identityCall, config);
  } catch (_) {
    identity = null;
  }
  ...
  try {
    view = await replayCall;
  } catch (error) {
    return NightjarViewData(
      status: NightjarViewStatus.unverified,
      statusMessage:
          'This wallet could not replay the Nightjar channel, so it is not '
...
/// [rust_nightjar.NjView] and a [NightjarIndexerStatus] and no native library.
NightjarViewData nightjarViewFrom({
  required rust_nightjar.NjView view,
  required NightjarIndexerStatus status,
  NightjarIdentityData? identity,
  NightjarChainTipSource chainTipSource = NightjarChainTipSource.wallet,
}) {
  final assets = _assetsOf(view);
  final live = status.live;

  NightjarViewData at(
    NightjarViewStatus viewStatus, {
    String? message,
    String? detail,
  }) {
    return NightjarViewData(
      ...
      assets: viewStatus == NightjarViewStatus.unverified ? const [] : assets,
  ...
  if (view.applied == 0 && view.ignored > 0) {
    return at(
      NightjarViewStatus.unverified,
      message: kNightjarNothingVerifiedText,
  ...
  if (indexerStateRoot != null && live.stateRootHeight == view.height) {
    if (!_sameRoot(indexerStateRoot, view.stateRoot)) {
      return at(
        NightjarViewStatus.unverified,
        message: kNightjarStateRootMismatchText,
        detail:
  ...
  if (live.height < view.height) {
    return at(
      ...
      message: kNightjarIndexerBehindText,
      detail:
  ...
  if (status.isStale) {
    return at(
      NightjarViewStatus.stale,
      message: kNightjarStaleText,
      detail: live.status,
    );
  }

  return at(NightjarViewStatus.ready);
}
...
String _ignoredSummary(rust_nightjar.NjView view) {
  const shown = 3;
...
BigInt? _declaredMaxSupply(rust_nightjar.NjAsset asset) {
...
NightjarIdentityData _identityOf(
  rust_nightjar.NjIdentity identity,
  NightjarConfig config,
) {
  return NightjarIdentityData(
    address: identity.address,
    networkLabel: nightjarNetworkLabel(config.networkName),
  );
}

List<NightjarAssetDetailData> _assetsOf(rust_nightjar.NjView view) {
  final assets = <NightjarAssetDetailData>[];
  for (final asset in view.assets) {
    ...
    assets.add(
      NightjarAssetDetailData(
        ...
        // enforced at section 6 step 6f). `NjAsset` does not carry it yet, so
        // this is null for every asset and every collection reads as uncapped.
        // That is the safe direction rather than a placeholder: step 6f lets a
        // wallet call a *capped* collection's count verified and forbids it for
        // an uncapped one, so an unread cap only ever costs a true sentence.
        ...
        declaredMetadata: [
          if (asset.uri.isNotEmpty)
            NightjarAssetFactData(label: 'URI', value: asset.uri),
        ],
        notes: notes,
      ),
    );
...
/// tip and the active account's seed, then delegates to [loadNightjarView].
///
/// The seed is read per refresh, after the network round trip, and zeroed
/// before either FFI result is awaited. Nightjar needs it at all because
/// ownership *is* decryption: there is no watch-only form of "which of these
/// notes are mine".
Future<NightjarViewData> loadNightjarViewFor(Ref ref) async {
  ...
  try {
    ...
    return NightjarViewData(
      status: NightjarViewStatus.notConfigured,
      statusMessage: 'Add a Nightjar indexer before loading assets.',
  ...
  // first sync reports one, which is the case [loadNightjarView] discloses
