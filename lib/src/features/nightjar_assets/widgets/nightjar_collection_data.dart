...
///
/// Nightjar has no NFT type and no collection object. A collection is a
...
const int kNightjarCollectionMinMembers = 2;

/// One `collection_id` and the members of it this view knows about.
class NightjarCollectionData {
  const NightjarCollectionData({
  ...
  ///
  /// Null for every collection today: see
  /// [NightjarAssetDetailData.collectionMaxSupply].
  final int? maxSupply;

  /// Whether a verifier enforces a ceiling on this collection.
  bool get isCapped => maxSupply != null;

  /// The members, ordered by [NightjarAssetDetailData.index] where the wallet
  ...
  /// Members this wallet holds right now. Private, like every Nightjar
...
class NightjarAssetsListing {
  const NightjarAssetsListing({
    required this.collections,
    required this.ungrouped,
  });

  const NightjarAssetsListing.empty()
    : collections = const [],
      ungrouped = const [];

  /// Grouped collections, ordered by [NightjarCollectionData.title] and then
  /// by `collection_id` so the order is total and survives a re-replay.
  final List<NightjarCollectionData> collections;
  ...
  final List<NightjarAssetDetailData> ungrouped;
  ...
  NightjarCollectionData? collectionById(String collectionId) {
...
///   [kNightjarCollectionMinMembers] members in this view.
...
NightjarAssetsListing groupNightjarCollections(
  List<NightjarAssetDetailData> assets,
) {
  final byCollection = <String, List<NightjarAssetDetailData>>{};
  ...
  final collections = <NightjarCollectionData>[];
  final grouped = <String>{};
  for (final entry in byCollection.entries) {
    if (entry.value.length < kNightjarCollectionMinMembers) continue;
    final members = [...entry.value]..sort(compareNightjarCollectionMembers);
    collections.add(
      NightjarCollectionData(
...
/// what [NightjarAssetDetailData.index] is for. The FFI carries it now, so the
...
int compareNightjarCollectionMembers(
  NightjarAssetDetailData a,
  NightjarAssetDetailData b,
) {
  final ai = a.index;
  final bi = b.index;
...
int? _collectionMaxSupply(List<NightjarAssetDetailData> members) {
  for (final member in members) {
...
int _compareCollections(NightjarCollectionData a, NightjarCollectionData b) {
  final at = a.title;
  final bt = b.title;
  if (at != null && bt != null) {
...
String? _derivedCollectionTitle(List<NightjarAssetDetailData> members) {
  String? prefix;
  for (final member in members) {
...
class NightjarCollectionRowData {
  const NightjarCollectionRowData({
    ...
    this.artwork = const NightjarCollectionArtworkData.notAccepted(),
    this.onTap,
  });
