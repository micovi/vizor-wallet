...
/// an index the *document* supplied. So [NyctisCollectionMetadata.memberAt]
...
/// relaxed — see `nyctis_metadata_reader.dart`.
library;

import 'dart:convert';
import 'dart:typed_data';

import '../services/nyctis_metadata_digest.dart';
import 'nyctis_asset_metadata.dart';

/// The schema family this document is written in. Section 3.1.
const String kNyctisCollectionSchemaFamily = 'nyctis-collection-metadata';
...
/// read best-effort.
const int kNyctisCollectionSchemaMajor = 1;

/// Section 3.1: `name` is at most 48 characters.
const int kNyctisCollectionMaxNameChars = 48;
...
const int kNyctisCollectionMaxDescriptionChars = 1000;

/// Section 3.4: at most 16 attributes per item, each string ≤ 48 characters.
const int kNyctisItemMaxAttributes = 16;
const int kNyctisItemAttributeMaxChars = 48;
...
const String kNyctisIndexToken = '{index}';
...
class NyctisCollectionItemTemplate {
  const NyctisCollectionItemTemplate({this.name, this.image});
...
class NyctisItemAttribute {
  const NyctisItemAttribute({required this.trait, required this.value});
...
class NyctisCollectionItemOverride {
  const NyctisCollectionItemOverride({
    required this.index,
    this.name,
    this.description,
    this.image,
  ...
  final List<NyctisItemAttribute> attributes;
}

/// One member of a collection, resolved against its **on-chain** index.
///
/// Produced by [NyctisCollectionMetadata.memberAt] and by nothing else, so
/// there is no way to build one of these without having said which index it
/// is for.
class NyctisCollectionMember {
  const NyctisCollectionMember({
    required this.index,
    required this.name,
  ...
  final NyctisItemImageRejection? imageRejection;
...
const int kNyctisDigestBytes = 32;
...
const int kNyctisDigestTableMaxCount = 16384;
...
/// measured.
const int kNyctisItemsDocumentMaxBytes = 2 * 1024 * 1024;

class NyctisCollectionDigestTable {
  const NyctisCollectionDigestTable({
    required this.uri,
    required this.b2,
    required this.count,
  });
  ...
  /// bytes; see [NyctisCollectionMetadata.resolveDigestTable].
  final int count;

  /// The exact byte length the table must have.
  int get expectedBytes => count * kNyctisDigestBytes;
}
...
/// them.
class NyctisCollectionItemsDocument {
  const NyctisCollectionItemsDocument({
    required this.uri,
    required this.b2,
    required this.bytes,
  });
  ...
  ///
  /// This is the member that has no counterpart on [NyctisCollectionDigestTable],
...
class NyctisCollectionMetadata {
  const NyctisCollectionMetadata({
    required this.item,
    this.revision = 0,
    this.name,
    this.description,
  ...
  final NyctisCollectionDigestTable? digestTable;
  ...
  final NyctisCollectionItemsDocument? itemsDocument;
  ...
  ///
  /// Typed as the *asset* document's [NyctisAssetLogoRef] on purpose. Section
  ...
  /// where the on-chain one does — `nyctis_collection_data.dart` holds the
  /// chain's cap and `nyctis_collection_mapper.dart` writes the sentence.
  final int? maxSupply;

  /// The template every member is resolved through.
  final NyctisCollectionItemTemplate item;
  ...
  final List<NyctisCollectionItemOverride> items;
  ...
  NyctisCollectionMember? memberAt(int index) {
    ...
    final templateName = item.name == null
        ? null
        : nyctisSubstituteIndex(item.name!, index);
    ...
    if (override?.image != null) {
      ...
      final result = nyctisResolveItemImage(item.image, index);
      image = result.uri;
      rejection = result.rejection;
    }

    return NyctisCollectionMember(
      index: index,
      name: resolvedName,
  ...
  /// unrepresentable.
  NyctisCollectionMetadata? resolveDigestTable(Uint8List bytes) {
    final table = digestTable;
    if (table == null) return null;
    if (bytes.length != table.expectedBytes) return null;
    if (!nyctisB2DigestMatches(table.b2, bytes)) return null;
    return NyctisCollectionMetadata(
      item: item,
      revision: revision,
      name: name,
  ...
  NyctisCollectionMetadata? resolveItemsDocument(Uint8List bytes) {
    final pointer = itemsDocument;
    if (pointer == null) return null;
    if (bytes.length != pointer.bytes) return null;
    if (!nyctisB2DigestMatches(pointer.b2, bytes)) return null;
  ...
  String? digestAt(int index) {
    ...
    if (table != null) {
      final start = index * kNyctisDigestBytes;
  ...
  NyctisCollectionItemOverride? overrideFor(int index) {
  ...
  /// [NyctisAssetMetadata.parseBytes] gives: the `#b2=` pin of
  /// `asset-metadata-v0.md` section 2.1 is over the exact bytes, so "the same
  /// document with a BOM" is a different document.
  factory NyctisCollectionMetadata.parseBytes(List<int> bytes) =>
      NyctisCollectionMetadata.fromJson(nyctisDecodeDocument(bytes));

  factory NyctisCollectionMetadata.fromJson(Map<String, Object?> json) {
    final major = nyctisSchemaMajor(
      json['schema'],
      kNyctisCollectionSchemaFamily,
    );
    if (major != kNyctisCollectionSchemaMajor) {
    ...
    if (item == null) {
      ...
      throw const NyctisMetadataFormatException('item is missing');
    }
    ...
    return NyctisCollectionMetadata(
      ...
      // drift.
      name: _text(json['name'], kNyctisCollectionMaxNameChars),
      description: _text(
        json['description'],
        kNyctisCollectionMaxDescriptionChars,
      ),
      // Section 3.5, through the section 4.2 parser rather than beside it.
      // A member this revision did not define — `mime`, `width`, `height` —
      // is dropped there for the reasons `NyctisAssetLogoRef` records, and
      // an unknown one is ignored, per `asset-metadata-v0.md` section 4.4.
      logo: nyctisAssetLogoRef(json['logo']),
  ...
  static NyctisCollectionDigestTable? _digestTable(Object? value) {
    ...
    if (count > kNyctisDigestTableMaxCount) return null;
    return NyctisCollectionDigestTable(uri: uri, b2: b2, count: count);
  }
  ...
  static NyctisCollectionItemsDocument? _itemsDocument(Object? value) {
    if (value is! Map<String, Object?>) return null;
    final uri = nyctisHttpsUri(value['uri']);
    if (uri == null) return null;
    final b2 = value['b2'];
    ...
    if (bytes > kNyctisItemsDocumentMaxBytes) return null;
    return NyctisCollectionItemsDocument(uri: uri, b2: b2, bytes: bytes);
  }
  ...
  static NyctisCollectionItemTemplate? _item(Object? value) {
    if (value is! Map<String, Object?>) return null;
    final name = value['name'];
    final image = value['image'];
    return NyctisCollectionItemTemplate(
      name: name is String && name.trim().isNotEmpty
          ? sanitizeNyctisMetadataText(name)
          : null,
  ...
  static List<String?> _digests(Object? value) {
    ...
    return List.unmodifiable([
      for (final entry in value)
        if (entry is String && isNyctisB2Digest(entry)) entry else null,
    ]);
  }

  static List<NyctisCollectionItemOverride> _items(Object? value) {
    if (value is! List) return const [];
    final items = <NyctisCollectionItemOverride>[];
    for (final entry in value) {
      ...
      items.add(
        NyctisCollectionItemOverride(
          index: index,
          name: _text(entry['name'], kNyctisCollectionMaxNameChars),
          description: _text(
            entry['description'],
            kNyctisCollectionMaxDescriptionChars,
          ),
          image: nyctisHttpsUri(entry['image']),
          digestB2: digest is String && isNyctisB2Digest(digest)
              ? digest
              : null,
  ...
  static List<NyctisItemAttribute> _attributes(Object? value) {
    if (value is! List) return const [];
    final attributes = <NyctisItemAttribute>[];
    for (final entry in value) {
      if (attributes.length >= kNyctisItemMaxAttributes) break;
      if (entry is! Map<String, Object?>) continue;
      final trait = _text(entry['trait'], kNyctisItemAttributeMaxChars);
...
NyctisItemImageResult nyctisResolveItemImage(String? template, int index) {
  if (template == null || template.isEmpty) {
    return const NyctisItemImageResult.rejected(
      NyctisItemImageRejection.absent,
    );
  }
  final substituted = nyctisSubstituteIndex(template, index);
  if (substituted.contains('{') || substituted.contains('}')) {
    return const NyctisItemImageResult.rejected(
      NyctisItemImageRejection.unresolvedToken,
    );
  }
  final uri = nyctisHttpsUri(substituted);
  if (uri == null) {
    return const NyctisItemImageResult.rejected(
      NyctisItemImageRejection.notHttps,
    );
  }
  return NyctisItemImageResult.resolved(uri);
}
...
String nyctisSubstituteIndex(String template, int index) =>
    template.replaceAll(kNyctisIndexToken, '$index');
