...
/// (`crates/nyctis-codec/src/asset.rs`, `MAX_URI`).
///
...
const int kNyctisUriMaxBytes = 255;
...
class NyctisMetadataPointer {
  const NyctisMetadataPointer({
  ...
  String get origin => nyctisUriOrigin(requestUri);

  @override
  String toString() => 'NyctisMetadataPointer($requestUri, pinned=$isPinned)';
}

/// The outcome of reading one `uri`: a pointer, or the reason there is none.
class NyctisPointerResult {
  const NyctisPointerResult.pointer(NyctisMetadataPointer this.pointer)
    : rejection = null;

  const NyctisPointerResult.rejected(NyctisPointerRejection this.rejection)
    : pointer = null;

  final NyctisMetadataPointer? pointer;
  final NyctisPointerRejection? rejection;
...
NyctisPointerResult readNyctisMetadataUri(String? raw) {
  ...
  return NyctisPointerResult.pointer(
    NyctisMetadataPointer(requestUri: uri.removeFragment(), digestB2: digest),
  );
}
