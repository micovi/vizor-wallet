...
/// one.
final nyctisAssetArtworkFetchProvider =
    FutureProvider.family<NyctisArtworkFetchOutcome?, String>((
      ref,
      assetId,
    ) async {
      ...
      try {
        final view = await ref.watch(nyctisAssetsViewProvider.future);
...
final nyctisAssetMetadataProvider =
    FutureProvider.family<NyctisAssetMetadataView?, String>((
      ref,
      assetId,
    ) async {
      final outcome = await ref.watch(
        nyctisAssetArtworkFetchProvider(assetId).future,
      );
...
///
/// The five states are in [NyctisArtworkStatus] and the reason they are five
...
final nyctisAssetLogoProvider = Provider.family<Uint8List?, String>((
  ref,
  assetId,
) {
  return ref.watch(nyctisAssetArtworkProvider(assetId)).bytes;
});
...
/// reached through [nyctisAssetArtworkProvider] from the tile that draws
...
final nyctisAssetLogosProvider = Provider<Map<String, Uint8List>>((ref) {
  // The home and activity feeds watch this unconditionally; a build without
  // the feature must not reach the acceptance set or the view from here.
  ...
  final grouped = ref.watch(nyctisGroupedMemberIdsProvider);
  final logos = <String, Uint8List>{};
  for (final entry in acceptance.accepted) {
    if (grouped.contains(entry.assetId)) continue;
    final bytes = ref.watch(nyctisAssetArtworkProvider(entry.assetId)).bytes;
...
bool nyctisAssetHasMetadataPointer(NyctisAssetDetailData? asset) {
  final uri = asset?.metadataUri;
