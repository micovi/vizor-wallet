...
///
/// * [NyctisArtworkStatus.verified] and [NyctisArtworkStatus.unpinned] are
...
/// * [NyctisArtworkStatus.refused] is the one that must never be silent. A
...
class NyctisArtworkData {
  ...
  const NyctisArtworkData.notAccepted()
    : status = NyctisArtworkStatus.notAccepted,
      bytes = null,
      reason = null,
      sourceOrigin = null,
  ...
  const NyctisArtworkData.pending()
    : status = NyctisArtworkStatus.pending,
      bytes = null,
      reason = null,
      sourceOrigin = null,
      documentPinned = false;

  final NyctisArtworkStatus status;

  /// Verified `image/png`, `image/jpeg` or `image/webp` bytes, or null.
  ///
  /// Non-null exactly when [status] is [NyctisArtworkStatus.verified] or
  /// [NyctisArtworkStatus.unpinned]. Whoever set it has already checked
  /// acceptance; a widget cannot, and must not try.
  final Uint8List? bytes;

  /// Why there are no bytes, when [status] is
  /// [NyctisArtworkStatus.refused]. Diagnostic — the copy a tile shows is
  ...
  bool get isPinned => status == NyctisArtworkStatus.verified;
  ...
  factory NyctisArtworkData.fromOutcome(NyctisArtworkFetchOutcome outcome) {
  ...
  static String? _origin(NyctisArtworkFetchOutcome outcome) =>
      outcome.assetView?.sourceOrigin ?? outcome.memberView?.sourceOrigin;

  static bool _documentPinned(NyctisArtworkFetchOutcome outcome) =>
  ...
  static NyctisMetadataAbandonReason? _reason(
...
enum NyctisCollectionArtworkSource {
  /// The document carried a `logo` (section 3.5) and these are its bytes.
  declared,

  /// The document carried none, so this is [NyctisCollectionArtworkData
...
class NyctisCollectionArtworkData {
  const NyctisCollectionArtworkData({
  ...
  final NyctisCollectionArtworkSource? source;

  /// The **on-chain** index of the member the face was taken from, when
  /// [source] is [NyctisCollectionArtworkSource.derived].
  ///
