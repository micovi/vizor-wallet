/// Plain view models for the Nyctis asset surfaces, plus the explicit
...
///
/// * **Balances are private.** A Nyctis note is only visible to the wallet
///   that can decrypt it, so there is no holder list and no circulating
///   supply. [NyctisAssetDetailData] therefore carries an *issued* supply
///   for public assets and nothing at all for private ones.
/// * **An asset may have no name.** An asset id is a hash; it only gains a
///   name when its issuer publishes an `ASSET` message. [NyctisAssetRowData
///   .name] being null is the normal case, not an error.
/// * **A received asset is invisible until it is [NyctisViewData
...
///   [NyctisViewData.pendingMessageCount] — which is a fact about the
...
/// turned into text by [formatNyctisAmount]. There is no floating point
...
/// How much of the Nyctis view the wallet was actually able to build.
enum NyctisViewStatus {
  /// No Nyctis indexer / channel is configured, so nothing has been read.
  notConfigured,
...
/// One row of the Nyctis assets list: what this wallet holds of one asset.
class NyctisAssetRowData {
  const NyctisAssetRowData({
  ...
  /// replay closes the view [NyctisViewData.finalityDepth] blocks below the
  ...
  /// state the replay can report. See [NyctisAssetDetailData.index] for why
...
class NyctisNoteRowData {
  const NyctisNoteRowData({
...
class NyctisAssetFactData {
  const NyctisAssetFactData({
    required this.label,
    required this.value,
    this.copyText,
  });
...
class NyctisAssetDetailData {
  const NyctisAssetDetailData({
    required this.assetId,
    required this.balance,
    required this.decimals,
    this.name,
  ...
  /// `collection_max_supply` on `NyAsset`, so nothing sets it and every
  ...
  final List<NyctisAssetFactData> declaredMetadata;

  /// The wallet's own notes of this asset. All of them are final; see
  /// [NyctisAssetRowData.noteCount].
  ...
  ///
  /// There is no separate NFT type in Nyctis. A unique item is an ordinary
  ...
  /// `Some(0)` case (`nyctis_view_loader.dart`), so this is never true of an
  ...
  NyctisAssetRowData toRowData({VoidCallback? onTap, Uint8List? logoBytes}) {
    return NyctisAssetRowData(
      assetId: assetId,
      name: name,
      symbol: symbol,
      balance: balance,
      decimals: decimals,
      // Unspent only, like the detail screen's count: `notes` is every note
      // the wallet has ever owned, so the activity feed can show what was
      // sent, and counting spent ones here made the list say "2 notes" for an
      // asset the detail screen says is made of 1.
      noteCount: notes.where((note) => !note.spent).length,
      isPublic: isPublic,
...
/// The whole Nyctis view for the active wallet: its identity, every asset
/// it holds, and how much of the channel the wallet actually managed to read.
class NyctisViewData {
  const NyctisViewData({
    required this.status,
    this.identity,
    this.assets = const [],
    this.finalityDepth = kNyctisDefaultFinalityDepth,
    ...
    this.indexerHeight,
    this.chainTipHeight,
    this.chainTipSource = NyctisChainTipSource.wallet,
    this.vkHash,
    this.statusMessage,
    this.statusDetail,
  });
  ...
  const NyctisViewData.notConfigured()
    : status = NyctisViewStatus.notConfigured,
      ...
      indexerHeight = null,
      chainTipHeight = null,
      chainTipSource = NyctisChainTipSource.wallet,
      vkHash = null,
      statusMessage = null,
      statusDetail = null;

  final NyctisViewStatus status;

  /// Null until the wallet has derived its Nyctis account.
  final NyctisIdentityData? identity;

  final List<NyctisAssetDetailData> assets;
  ...
  final NyctisChainTipSource chainTipSource;
  ...
  bool get isReady => status == NyctisViewStatus.ready;

  bool get isConfigured => status != NyctisViewStatus.notConfigured;

  /// True when the indexer answered but the wallet could not verify what it
  /// said. Retrying does not help; the fix is in settings.
  bool get isUnverified => status == NyctisViewStatus.unverified;

  /// True when the view was closed against a tip the indexer supplied.
  bool get borrowedChainTip => chainTipSource == NyctisChainTipSource.indexer;

  /// The asset with this id, or null when the view does not hold it.
  NyctisAssetDetailData? assetById(String assetId) {
...
/// Nyctis's finality depth on the devnet this proof of concept targets: a
/// received asset is invisible until it is this many blocks deep.
const int kNyctisDefaultFinalityDepth = 10;

/// Characters of an asset id kept at each end when it is truncated.
const int kNyctisAssetIdAffixLength = 6;
...
String formatNyctisAmount(BigInt amount, int decimals) {
...
/// from [formatNyctisAmount]; anything else — a sign, an exponent, a second
/// separator, more fraction digits than the asset has — is rejected rather
/// than repaired.
BigInt? parseNyctisAmount(String text, int decimals) {
...
bool nyctisAmountHasTooManyDecimals(String text, int decimals) {
  final raw = text.trim().replaceAll(',', '');
...
String truncateNyctisAssetId(
  String assetId, {
  int affixLength = kNyctisAssetIdAffixLength,
}) {
