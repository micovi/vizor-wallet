/// The Nyctis receive block: QR, full address, copy action, and the one
...
import 'nyctis_asset_row_mapper.dart';
import 'nyctis_assets_feed.dart';

class NyctisReceivePanel extends StatelessWidget {
  const NyctisReceivePanel({
    required this.address,
    this.networkLabel,
    this.qrSize = 220,
    this.maxWidth = kNyctisCardWidth,
    super.key,
  });

  /// The wallet's bech32m Nyctis address, or null when the wallet has not
  /// derived its Nyctis identity yet.
  ...
  Widget build(BuildContext context) {
    ...
    if (address == null || address.isEmpty) {
      return const NyctisMessageCard(
        key: ValueKey('nyctis_receive_no_identity'),
        text: kNyctisNoIdentityText,
    ...
    return ConstrainedBox(
      ...
      child: Column(
        ...
        children: [
          RequestQrSurface(
            key: const ValueKey('nyctis_receive_qr'),
            data: address,
            size: qrSize,
          ),
