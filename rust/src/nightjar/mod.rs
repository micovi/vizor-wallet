//! Nightjar: a client-verified asset overlay on Zcash.
//!
//! Nightjar's messages ride inside ordinary Ironwood shielded memos addressed to a public
//! *channel*, and every participant replays that channel to reach the same state. Nothing about
//! it is enforced by consensus, so a wallet that wants to believe a Nightjar balance has to
...
//!
//! The three crates linked here (`nightjar-codec`, `nightjar-state`, `nightjar-zk`) carry no
//! Zcash dependency. `nightjar-scanner`, `nightjar-chain` and `nightjar-wallet` deliberately are
...
//! `zcash_client_sqlite` migration sets over one database file. Every Zcash-shaped job Nightjar
//! needs is done by this wallet's own stack instead, and the channel-scanning half is done by the
//! Nightjar indexer over HTTP (see `docs/NIGHTJAR-POC.md`).
//!
//! `nightjar-scanner` not being linkable has one further consequence worth naming here, because
