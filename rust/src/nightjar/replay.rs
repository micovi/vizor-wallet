//! Offline replay of a Nightjar channel from messages somebody else fetched.
//!
//! The reference implementation (`nightjar-scanner`) does two jobs in one pass: it finds the
...
//! finding and reassembling the messages is the Nightjar indexer's, fetched over HTTP from Dart,
...
//! module had none and refused every claim ([`nightjar_state::NoZec`]). It no longer does:
//! [`crate::nightjar::carrier`] fetches that transaction from this wallet's own database or its
//! own lightwalletd and proves it carried the message, and [`crate::nightjar::zec`] — a port of
...
use nightjar_codec::transport::{
    channel_id, msg_id, ChannelId, Delivered, Location, FINALITY_DEPTH, JOURNAL_DEPTH,
};
use nightjar_state::{AppliedNote, Outcome, State};
use nightjar_zk::tree::PoseidonHasher;
use nightjar_zk::verifier::Groth16Verifier;

use crate::nightjar::carrier::{self, Carriers, TxSources};
use crate::nightjar::network::NightjarNetwork;
use crate::nightjar::zec::{self, PaidClaims};
...
pub struct Message {
    ...
    /// message's is [`crate::nightjar::carrier::bind`], which finds the message's own fragments
...
/// binding is `msg_id = BLAKE2b-256("nightjar.msg.v0" ‖ channel_id ‖ [VERSION, kind] ‖ count_le ‖
/// body)` (transport-envelope-v0 section 3), and `nightjar_state` leans on it hard: the comment
...
pub struct MessageOutcome {
    ...
    /// carrying transaction was never in hand. [`crate::nightjar::zec::PaidClaims`] keeps the
...
/// A replayed channel, plus what the recovery pass in [`crate::nightjar::owned`] needs to read
...
///
/// Restated here rather than imported because `nightjar_scanner::canonical_height` lives in the
...
/// `nightjar_scanner::channel_id_for_uivk`). Hashing the string would be the easier mistake and
...
pub fn channel_id_for_uivk(network: WalletNetwork, uivk: &str) -> Result<ChannelId, String> {
    ...
    Ok(channel_id(network.nj_wire(), 0, &payload))
}
...
/// claims (spec `transition-v0.md` section 6 step 6b, F24). That is [`crate::nightjar::zec`], a
/// port of `nightjar-scanner`'s `PaidClaims` — the reference implementation cannot be linked into
...
/// each claim needs: [`crate::nightjar::carrier`] fetches it from this wallet's own database or
...
pub fn replay_using(
    ...
    // from the indexer.
    //
    // The bound has a floor as well as a ceiling. `nightjar-scanner` is handed the block range
    ...
    let mut paid = PaidClaims::new(network, network.nj_wire(), &carrying_txs);
    ...
    for msg in &messages {
        ...
        // [`crate::nightjar::carrier::bind`] found the message's whole framing inside the
    ...
    // anyone else computes. Measured on the devnet: 23 anchors and root `d90117d6…` at height
    // 621 where the last message sits, against 1 anchor and `c67b838d…` at canonical height
    // 832 after 211 empty blocks, which is what the reference scanner and the indexer report
...
mod tests {
    ...
    use crate::nightjar::testdata::claims;
    ...
    fn the_channel_id_is_over_the_bech32m_payload() {
        ...
        let want = channel_id(WalletNetwork::Regtest.nj_wire(), 0, &payload);
        assert_eq!(hex::encode(want), testdata::CHANNEL_ID);
        let from_string = channel_id(
            WalletNetwork::Regtest.nj_wire(),
            0,
    ...
    /// devnet was mined 210 empty blocks to create it — recorded where the channel stood, the two
    ...
    fn a_quiet_channel_prunes_its_anchors_at_the_canonical_height() {
        ...
        // Every anchor still within `ANCHOR_WINDOW` of height 621, which on this channel means
        // every applied transition from height 421 on. `/api/anchors?as_of=621&live=1` reports
        // the same 23, which is the indexer applying the same rule to its own history.
        assert_eq!(at_last_message.state.anchor_count(), 23);
        assert_eq!(
    ...
    /// **The acceptance test.** The live devnet channel, 31 messages, two of them carrying a ZEC
    /// claim — and this wallet must land on the two roots the indexer publishes and accept the
    /// same 25 messages.
    ///
    /// Before the oracle existed this replay accepted **11**. The arithmetic of the gap is the
    ...
    /// since `nightjar::zec` is a *port* of the scanner's `PaidClaims` rather than a call into
    ...
    fn the_view_reports_the_hash_of_the_key_it_verified_against() {
        ...
        assert_eq!(
            ...
            hex::encode(nightjar_zk::prover::vk_hash_bytes(testdata::vk())),
        );
    }
