//! Getting the Zcash transaction that carried a Nyctis message, and proving that it did.
//!
//! [`crate::nyctis::zec`] can answer "did this transaction pay this claim?" only once somebody
//! puts the transaction in its hand. This module is that somebody. It is the piece the indexer
//! cut removed: `nyctis-scanner` is handed the block it is scanning, so the carrying
...
//! * the Nyctis fragments that come out must cover slots `0..count-1` of *this* `msg_id`, with
//!   payloads equal to the slices of *this* body;
//! * and the body hashes to `msg_id` ([`crate::nyctis::replay::check_binding`], already run).
//!
...
//! ([`crate::nyctis::zec::claim_carrying`] selects them and [`resolve`] dedupes on txid), and
...
//! trips, and a lightwalletd that has gone away must not hang the Nyctis screen.

use std::collections::BTreeMap;
use std::sync::Arc;
use std::time::{Duration, Instant};

use nyctis_codec::transport::{parse, Location, MsgId, FRAGMENT_PAYLOAD};
...
use crate::nyctis::zec::CarryingTxs;
...
impl Carriers {
    /// The view [`crate::nyctis::zec::PaidClaims`] wants: bytes and reasons per
    /// `(height, tx_index)`.
...
pub fn bind(
    ...
    let Some(bundle) = tx.ironwood_bundle() else {
        return Err("the fetched transaction has no Ironwood bundle, so it carried no Nyctis \
                    memo"
            .to_string());
    };
...
///
/// `wanted` is [`crate::nyctis::zec::claim_carrying`]'s output: the messages that actually make
...
impl<'a> LwdFetcher<'a> {
    ...
    /// Source 2: `GetTransaction`. Blocking, on a runtime of its own, the way this crate's other
    /// synchronous foreground FRB entry points reach lightwalletd (`api/sync.rs`).
    ///
    /// The replay runs in the foreground (the Nyctis and Activity screens), so the channel goes
    /// through [`crate::wallet::sync_engine::open_lwd_channel`]: it follows the user's network
    /// route and fails closed while Tor is starting or broken. The pinned background opener is
    /// for the iOS background lane only (AGENTS.md, "Tor and the network route"); used here it
    /// sent every carrier lookup — naming exactly the transactions this wallet cares about — in
    /// clear with Tor on.
    fn get(&mut self, txid: &[u8; 32]) -> Result<Arc<Vec<u8>>, String> {
        ...
        runtime.block_on(async move {
            let mut client = crate::wallet::sync_engine::open_lwd_channel(&url)
                .await
...
mod tests {
    use super::*;
    use crate::nyctis::testdata::claims as testdata;
    ...
    /// The carrier lookup is foreground traffic, so it follows the network route: with Tor
    /// wanted and not running it fails closed instead of connecting in clear.
    #[test]
    fn lightwalletd_lookup_fails_closed_when_tor_is_unavailable() {
        ...
        let err = LwdFetcher::new(&url)
            .get(&[7u8; 32])
            .expect_err("Tor is wanted and not running");

        assert!(err.contains("network privacy blocked"), "{err}");
        assert!(
            matches!(server.accept(), Err(e) if e.kind() == std::io::ErrorKind::WouldBlock),
            "the lookup reached the socket in clear"
        );
    }

    // A supplemental source guard in the style of the voting factory guard
    // (`wallet/voting/network_clients.rs`): the pinned direct opener belongs to the iOS
    // background lane alone, so nothing outside `ffi.rs` (and the opener's own module and
    // re-export) may name it.
    #[test]
    fn pinned_background_opener_stays_in_the_background_lane() {
        ...
        let allowed = [
            "src/ffi.rs",
            "wallet/sync_engine/lwd.rs",
            "wallet/sync_engine/mod.rs",
            "nyctis/carrier.rs",
        ];
        for path in files {
            ...
            let code: String = source
                ...
                .join("\n");
            assert!(
                !code.contains("open_background_direct_lwd_channel"),
                "{} uses the pinned background lightwalletd opener; foreground paths must \
                 use open_lwd_channel / open_lwd_channel_with_cancel / open_isolated_lwd_channel",
                path.display()
            );
        }
        // This file is exempt only because it names the opener in this guard; its code must
        // not call it.
        let this_file = include_str!("carrier.rs");
        let before_tests = this_file.split("#[cfg(test)]").next().unwrap();
        assert!(!before_tests.contains("open_background_direct_lwd_channel("));
    }
}
