...
use nyctis_state::PublishedNote;
use nyctis_zk::keys::Account;
use nyctis_zk::note::Note;
use nyctis_zk::{fq_from_bytes, Fq};

use crate::nyctis::replay::ChannelView;
...
pub struct OwnedNote {
    ...
    pub asset_id: [u8; 32],
    pub amount: u64,
    /// The policy rendered as its text form (`nyctis-codec`'s `Display`), e.g. `pk(<ak>)`.
    pub policy: String,
    pub source: Source,
    ...
    /// view produced by [`crate::nyctis::replay`] contains, `None` on a note that is `spent`
...
pub struct AssetSummary {
    ...
    /// The cap of [`AssetSummary::collection_id`]: how many members that collection may ever
    /// have, where `Some(0)` means uncapped, as on the wire. `None` when no applied *public*
    /// issuance disclosed it — the same absence as `collection_id` and `max_supply`.
    ///
    /// **A fact every verifier enforces, not a number the issuer wrote down.** It is a preimage
    /// component of `collection_id` (`transition-v0.md` section 5) and of `terms`, and `State`
    /// ignores a public issuance whose `index` is not strictly below a non-zero cap (section 6
    /// step 6f). So members of one collection cannot disagree about it, and a replayed channel
    /// never holds a member at or above it — which is what lets a wallet present a capped
    /// collection's count as verified, and forbids it for an uncapped one.
    pub collection_max_supply: Option<u64>,
    pub balance: u64,
    pub note_count: u32,
}
...
impl Spendable {
    ...
    pub fn note_view(&self) -> nyctis_codec::policy::NoteView {
        nyctis_codec::policy::NoteView {
...
/// ([`asset_summaries`]) and the spend path (`crate::nyctis::pay`, through
...
fn recover(view: &ChannelView, account: &Account) -> Vec<Recovered> {
    ...
    for msg in &view.messages {
        ...
        for r in nyctis_zk::transition::recover(&tx, account) {
...
pub fn asset_summaries(view: &ChannelView, notes: &[OwnedNote]) -> Vec<AssetSummary> {
    // `collection_id`, `max_supply`, `index` and `collection_max_supply` from the bodies of the
    // applied public issuances.
    let mut minted: BTreeMap<[u8; 32], ([u8; 32], u64, u32, u64)> = BTreeMap::new();
    for msg in &view.messages {
        ...
        // First issuance wins, and it is sound for all **four** of these, not just two.
        ...
        minted.entry(asset).or_insert((
            collection,
            tx.max_supply,
            tx.index,
            tx.collection_max_supply,
        ));
    }
    ...
    ids.into_iter()
        .map(|asset_id| {
            let (collection_id, max_supply, index, collection_max_supply) =
                match minted.get(&asset_id).copied() {
                    Some((c, m, i, cap)) => (Some(c), Some(m), Some(i), Some(cap)),
...
mod tests {
    ...
    fn the_demo_seed_owns_the_known_devnet_note() {
        ...
        let account = crate::nyctis::keys::account(&testdata::demo_seed());
    ...
    fn a_spent_note_names_the_message_that_spent_it() {
        let view = testdata::replay_fixture();
        let account = crate::nyctis::keys::account(&testdata::issuer_seed());
        let notes = owned_notes(&view, &account);
    ...
    fn the_two_sides_of_one_payment_agree() {
        ...
        let issuer = owned_notes(
            &view,
            &crate::nyctis::keys::account(&testdata::issuer_seed()),
        );
        ...
        let demo = owned_notes(
            &view,
            &crate::nyctis::keys::account(&testdata::demo_seed()),
        );
    ...
    fn a_stranger_replaying_the_same_channel_owns_nothing() {
        let view = testdata::replay_fixture();
        let account = crate::nyctis::keys::account(b"not the demo seed");
    ...
    fn the_balance_counts_unspent_notes_only() {
        ...
        let demo = crate::nyctis::keys::account(&testdata::demo_seed());
