...
//! had no answer to it at all — [`nyctis_state::NoZec`] was passed to `State::apply`, which
...
//!
//! The reference implementation is `PaidClaims` in `nyctis-scanner`, which the indexer and the
//! CLI both use. **It cannot be linked here.** `nyctis-scanner` depends on `zcash_keys`,
...
//! That constraint is why the fork links only `nyctis-codec`, `-state` and `-zk`, the three
//! Nyctis crates with no Zcash dependency at all.
//!
//! So the logic below is a **port**, function for function, of
//! `crates/nyctis-scanner/src/lib.rs` (`claim_account`, `uivk_string`, `paid_to`,
...
//! transaction ([`crate::nyctis::carrier`]), which can fail — no lightwalletd, a pruned
...
use nyctis_codec::transition::{Transition, ZecClaim};
use nyctis_codec::transport::{Location, Network as WireNetwork, KIND_TRANSITION};
use nyctis_state::ZecOracle;
...
///
/// Ported from `nyctis_scanner::uivk_string`. The HRPs are the protocol's, not this wallet's
...
/// what feeds it.
///
/// Ported from `nyctis_scanner::claim_account`.
...
///
/// Ported from `nyctis_scanner::paid_to`. `saturating_add` rather than `+` for the same reason
...
/// distinction on the side and [`crate::nyctis::replay`] folds it back into the outcome.
...
/// Carrying transactions, per `(height, tx_index)`, as [`crate::nyctis::carrier`] resolved
...
impl<'a> PaidClaims<'a> {
    ...
    ///
    /// `BranchId::for_height` mirrors the scanner. It has no effect on anything Nyctis carries:
    /// a Nyctis memo rides in an Ironwood action, which only exists in a v6 transaction, and
    /// `Transaction::read` ignores the branch id for v5 and v6 (it is only consulted to pick the
    /// v4 reader). It is passed anyway so that a diff against `nyctis-scanner` stays clean.
    ...
    /// only mean a transaction with no Nyctis memo. Here it is the fail-closed path, so it is
