...
import 'package:zcash_wallet/src/features/nightjar_assets/providers/nightjar_collection_artwork_provider.dart';
...
List<NightjarAssetDetailData> _pieces({int? collectionMaxSupply}) => [
...
Uint8List _documentBytes({
  ...
  final digest = nightjarB2Digest(kOnePixelPng);
...
FakeNightjarTransport _transport({
  ...
  if (logoBytes != null)
    _logoUri: NightjarHttpReply(statusCode: 200, body: logoBytes),
});
...
Map<String, Object?> _logoMember({Uint8List? pinTo}) => {
  ...
  if (pinTo != null) 'b2': nightjarB2Digest(pinTo),
...
final _collectionFace = find.byKey(kNightjarCollectionArtworkImageKey);
...
final _tilePictures = find.byKey(const ValueKey('nightjar_logo_image'));

/// The sentence the header puts under the collection's name.
String _provenance(WidgetTester tester) => tester
    .widget<Text>(find.byKey(const ValueKey('nightjar_collection_provenance')))
    .data!;
...
NightjarAssetAcceptance _acceptAll() => NightjarAssetAcceptance([
  for (var i = 0; i < _memberCount; i++)
    NightjarAcceptedAsset(assetId: _pieceId(i)),
]);

Future<FakeNightjarTransport> _pumpCollection(
  WidgetTester tester, {
...
void main() {
  group('a collection document fills the grid', () {
    testWidgets('ten accepted pieces draw ten pictures from one document', (
      ...
      expect(find.text(kNightjarArtworkNotAcceptedTileText), findsNothing);
      ...
      expect(
        fake.requested.where((uri) => uri != _documentUri).toSet(),
    ...
    testWidgets('a pinned piece shows no unpinned badge', (tester) async {
      ...
      expect(
        find.byKey(const ValueKey('nightjar_artwork_unpinned_badge')),
        findsNothing,
      );
    });
    ...
    testWidgets('a digest mismatch says so rather than saying nothing', (
      ...
      await _pumpCollection(
        tester,
        acceptance: _acceptAll(),
        transport: FakeNightjarTransport({
          _documentUri: NightjarHttpReply(
            statusCode: 200,
            body: _documentBytes(),
          ),
      ...
      expect(find.byKey(const ValueKey('nightjar_logo_image')), findsNothing);
      expect(
        find.text(kNightjarArtworkRefusedTileText),
        findsNWidgets(_memberCount),
      );
      expect(find.text(kNightjarArtworkNotAcceptedTileText), findsNothing);
    });
  ...
  group('the request set is a fact about the collection, not the wallet', () {
    testWidgets('accepting one piece warms every piece, accepted or not', (
      ...
      final fake = await _pumpCollection(
        ...
        acceptance: NightjarAssetAcceptance([
          NightjarAcceptedAsset(assetId: _pieceId(4)),
        ]),
      );
    ...
    testWidgets('and still draws only the piece that was accepted', (
      ...
      expect(
        find.text(kNightjarArtworkNotAcceptedTileText),
    ...
    test('the warm-up stops at its byte cap and says it is incomplete', () async {
      // Twenty quarter-megabyte images is 5 MiB, past
      // [kNightjarCollectionWarmupMaxBytes]. Past the cap the tail falls back
      ...
      final assets = [
        for (var i = 0; i < members; i++) _piece(i),
      ];
      final fake = FakeNightjarTransport({
        ...
        for (var i = 0; i < members; i++)
          _imageUri(i): NightjarHttpReply(statusCode: 200, body: fat),
      });
    ...
    test('a collection that grew is warmed across all its documents', () async {
      ...
      final fake = FakeNightjarTransport({
        ...
        second: NightjarHttpReply(
          statusCode: 200,
          body: _documentBytes(digestCount: 0),
        ),
        for (var i = 0; i < 8; i++)
          _imageUri(i): NightjarHttpReply(statusCode: 200, body: kOnePixelPng),
      });
    ...
    test('stops at the document bound and says which cap it hit', () async {
      ...
      final members = kNightjarCollectionMaxDocuments + 10;
      ...
      expect(warmup.documents, kNightjarCollectionMaxDocuments);
      ...
      expect(warmup.complete, isFalse);
      expect(warmup.requested, kNightjarCollectionMaxDocuments);
      expect(
        fake.requested.where((u) => u.path.endsWith('.json')).length,
        kNightjarCollectionMaxDocuments,
  ...
  group('the collection has a face of its own (section 3.5)', () {
    ...
    testWidgets('no logo and nothing accepted draws the empty frame', (
      ...
      expect(
        find.byKey(const ValueKey('nightjar_collection_artwork')),
        findsOneWidget,
      );
      // And nothing was said about where a picture came from, because there
      // is none.
      expect(
        find.byKey(const ValueKey('nightjar_collection_provenance')),
        findsNothing,
      );
      expect(fake.requested, isEmpty);
    });
    ...
    testWidgets('the collection id is beside the name, always', (tester) async {
      ...
      await _pumpCollection(tester, acceptance: _acceptAll());
      expect(
        find.byKey(const ValueKey('nightjar_collection_header_id')),
        findsOneWidget,
      );
      expect(
        tester
            ...
            .data,
        truncateNightjarAssetId(_collectionId),
      );
    });
    ...
    testWidgets('a collection row draws the face and marks it derived', (
        ...
        MaterialApp(
          home: AppTheme(
            ...
            child: Center(
              child: SizedBox(
                width: 396,
                child: NightjarCollectionRow(
                  row: NightjarCollectionRowData(
                    collectionId: _collectionId,
                    title: 'Phases of one night',
                    subtitle: truncateNightjarAssetId(_collectionId),
                    countText: '10 pieces · you hold 3',
                    ownedText: '3',
                    ownedLabel: 'held',
                    artwork: NightjarCollectionArtworkData(
      ...
      expect(_collectionFace, findsOneWidget);
      expect(
        find.byKey(const ValueKey('nightjar_collection_derived_badge')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('nightjar_collection_unpinned_badge')),
        findsOneWidget,
      );
      // Section 5 on the row too.
      expect(
        find.textContaining(truncateNightjarAssetId(_collectionId)),
        findsOneWidget,
      );
    });
  ...
  group('the five tile states are five different tiles', () {
    ...
    testWidgets('not accepted', (tester) async {
      await pumpTile(tester, const NightjarArtworkData.notAccepted());
      expect(find.text(kNightjarArtworkNotAcceptedTileText), findsOneWidget);
      expect(find.byKey(const ValueKey('nightjar_logo_image')), findsNothing);
    });

    testWidgets('pending', (tester) async {
      await pumpTile(tester, const NightjarArtworkData.pending());
      expect(find.text(kNightjarArtworkPendingTileText), findsOneWidget);
    });
    ...
    testWidgets('refused', (tester) async {
      ...
      expect(find.text(kNightjarArtworkRefusedTileText), findsOneWidget);
      expect(find.byKey(const ValueKey('nightjar_logo_image')), findsNothing);
    });
    ...
    test('and their copy is all distinct', () {
      ...
      final texts = [
        for (final status in NightjarArtworkStatus.values)
          nightjarArtworkTileText(status),
  ...
  group('the document cap against the cap in the collection id', () {
    ...
    String footnote(WidgetTester tester) {
      final card = find.byKey(const ValueKey('nightjar_collection_facts'));
      return tester
    ...
    final disagreement = find.byKey(
      const ValueKey('nightjar_collection_cap_disagreement'),
    );
    ...
    testWidgets('an uncapped collection keeps the old footnote and says '
      ...
      expect(footnote(tester), kNightjarCollectionCountNote);
    ...
    testWidgets('a capped collection counts against the cap', (tester) async {
      ...
      expect(footnote(tester), kNightjarCappedCollectionCountNote);
  ...
  group('provenance says where it came from and what pins it', () {
    test('a verified image names the digest', () {
      final text = nightjarArtworkProvenanceText(
        NightjarArtworkData(
          status: NightjarArtworkStatus.verified,
          bytes: kOnePixelPng,
    ...
    test('a state with no picture has nothing to say about its origin', () {
      expect(
        nightjarArtworkProvenanceText(const NightjarArtworkData.pending()),
        isNull,
      );
      expect(
        nightjarArtworkDocumentPinText(
