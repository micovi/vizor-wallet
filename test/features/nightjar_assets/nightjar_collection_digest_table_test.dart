...
Uint8List _table(int count) {
  final bytes = Uint8List(count * kNightjarDigestBytes);
  for (var i = 0; i < count; i++) {
    for (var b = 0; b < kNightjarDigestBytes; b++) {
      bytes[i * kNightjarDigestBytes + b] = (i * 31 + b * 7) & 0xFF;
    }
  }
  return bytes;
}
...
String _digestAt(Uint8List table, int index) => base64Url
    .encode(
      Uint8List.sublistView(
        table,
        index * kNightjarDigestBytes,
        (index + 1) * kNightjarDigestBytes,
      ),
    )
    .replaceAll('=', '');
...
Map<String, Object?> _pointer(Uint8List table) => {
  ...
  'b2': nightjarB2Digest(table),
  'count': table.length ~/ kNightjarDigestBytes,
};
...
void main() {
  group('section 3.3.1 — the pointer', () {
    test('parses uri, b2 and count', () {
      ...
      final collection = NightjarCollectionMetadata.fromJson(
    ...
    test('refuses a count past the section 3.2 ceiling', () {
      final pointer = _pointer(_table(10))
        ..['count'] = kNightjarDigestTableMaxCount + 1;
      final collection = NightjarCollectionMetadata.fromJson(
        _document(digestTable: pointer),
      );
      ...
      expect(collection.digestTable, isNull);
      expect(kNightjarDigestTableMaxCount * kNightjarDigestBytes, 512 * 1024);
    });
    ...
    test('refuses a b2 that is not 43 base64url characters', () {
      for (final bad in ['short', 'x' * 44, '${'a' * 42}+', '${'a' * 42}/']) {
        ...
        expect(
          NightjarCollectionMetadata.fromJson(
            _document(digestTable: pointer),
          ).digestTable,
          isNull,
  ...
  group('section 3.3.1 — resolving', () {
    ...
    test('resolveDigestTable is null when there is no pointer', () {
      final collection = NightjarCollectionMetadata.fromJson(_document());
    ...
    test('resolving leaves the original document untouched', () {
      ...
      final original = NightjarCollectionMetadata.fromJson(
        _document(digestTable: _pointer(table)),
      );
  ...
  group('section 3.3.1 — the two shapes are mutually exclusive', () {
    test('a document carrying both uses the table', () {
      ...
      final collection = NightjarCollectionMetadata.fromJson(
        _document(
          ...
          digests: [nightjarB2Digest(utf8.encode('something else'))],
        ),
      ).resolveDigestTable(table)!;
    ...
    test('inline digests still work, unchanged', () {
      ...
      final digest = nightjarB2Digest(utf8.encode('one'));
      final collection = NightjarCollectionMetadata.fromJson(
  ...
  group('through the fetcher', () {
    ...
    (FakeNightjarTransport, Uint8List) serve() {
      final pngDigest = nightjarB2Digest(kOnePixelPng);
      final raw = base64Url.decode('$pngDigest=');
      final table = Uint8List(400 * kNightjarDigestBytes);
      for (var i = 0; i < 400; i++) {
        table.setRange(i * kNightjarDigestBytes, (i + 1) * kNightjarDigestBytes, raw);
      }
      ...
      return (
        FakeNightjarTransport({
          ...
          tableUri: NightjarHttpReply(statusCode: 200, body: table),
          for (var i = 0; i < 400; i++)
            imageUri(i): NightjarHttpReply(statusCode: 200, body: kOnePixelPng),
        }),
        table,
      );
    }
    ...
    test('a table the host tampers with unpins the whole collection', () async {
      ...
      final fake2 = FakeNightjarTransport({
        ...fake.replies,
        tableUri: NightjarHttpReply(statusCode: 200, body: tampered),
      });
      final fetcher = NightjarAssetMetadataFetcher(transport: fake2);
  ...
  group('section 3.4.1 — the items document', () {
    ...
    test('parses uri, b2 and the declared length', () {
      final c = NightjarCollectionMetadata.fromJson(
    ...
    test('refuses a body whose b2 does not match', () {
      final pointer = itemsPointer(body)
        ..['b2'] = nightjarB2Digest(utf8.encode('other'));
      final c = NightjarCollectionMetadata.fromJson(
    ...
    test('refuses a declaration past the 2 MiB ceiling, before fetching', () {
      final pointer = itemsPointer(body)
        ..['bytes'] = kNightjarItemsDocumentMaxBytes + 1;
      final c = NightjarCollectionMetadata.fromJson(
        _document(extra: {'items_document': pointer}),
      );
    ...
    test('ten thousand traits fit where inline items reach 81', () {
      ...
      expect(many.length, lessThan(kNightjarItemsDocumentMaxBytes));
    });
  });
}
