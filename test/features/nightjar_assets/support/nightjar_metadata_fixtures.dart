/// Local fixtures for the Nightjar issuer-metadata tests.
///
...
/// [FakeNightjarTransport] rather than against a live host. That is also the
...
/// A scripted [NightjarMetadataTransport] that records what was asked for.
///
/// Recording matters more than replying: most of these tests assert on the
/// requests that were *not* made.
class FakeNightjarTransport implements NightjarMetadataTransport {
  FakeNightjarTransport(this.replies, {this.onRequest});

  final Map<Uri, NightjarHttpReply> replies;
  ...
  Future<NightjarHttpReply> get(
    ...
    if (reply == null) {
      return NightjarHttpReply(statusCode: 404, body: Uint8List(0));
    }
    return reply;
  }
}
