...
import 'package:zcash_wallet/src/features/nyctis_assets/models/nyctis_metadata_pointer.dart';
...
const _document = {
  ...
  'description': 'A demonstration asset on the Nyctis regtest devnet.',
  'logo': {
    'uri': 'https://example.invalid/ny/gold.png',
    'mime': 'image/png',
    'width': 512,
    'height': 512,
  },
...
void main() {
  group('NyctisAssetMetadata.parse', () {
    test('reads the section 4 example', () {
      final metadata = NyctisAssetMetadata.parse(jsonEncode(_document));

      expect(
        metadata.description,
        'A demonstration asset on the Nyctis regtest devnet.',
      );
      expect(
        metadata.logo!.uri.toString(),
        'https://example.invalid/ny/gold.png',
      );
    ...
    test('rejects an unrecognized schema rather than guessing at members', () {
      expect(
        () => NyctisAssetMetadata.parse(
          jsonEncode({..._document, 'schema': 'nyctis-asset-metadata/2'}),
        ),
        throwsA(isA<NyctisMetadataFormatException>()),
      );
    });

    test('rejects a document with no schema', () {
      expect(
        () => NyctisAssetMetadata.parse(jsonEncode({'description': 'hi'})),
        throwsA(isA<NyctisMetadataFormatException>()),
      );
    });
    ...
    test('rejects a top level that is not an object', () {
      ...
      expect(
        () => NyctisAssetMetadata.parse('not json'),
        throwsA(isA<NyctisMetadataFormatException>()),
      );
    });
    ...
    test('rejects a UTF-8 BOM, because the digest is over exact bytes', () {
      ...
      expect(
        () => NyctisAssetMetadata.parseBytes(bytes),
        throwsA(isA<NyctisMetadataFormatException>()),
      );
    });

    test('ignores unknown members at every level (section 4.4)', () {
      final metadata = NyctisAssetMetadata.parse(
        jsonEncode({
          'schema': 'nyctis-asset-metadata/1',
          'description': 'kept',
    ...
    test(
      ...
      () {
        ...
        expect(
          NyctisAssetMetadata.parse(
            jsonEncode({'schema': 'nyctis-asset-metadata/1'}),
          ).isEmpty,
          isTrue,
        );
      },
    ...
    test('truncates description at 1 000 characters', () {
      ...
      expect(
        ...
        kNyctisMetadataMaxDescriptionChars,
      );
    });
    ...
    test('keeps at most 16 links and skips malformed entries', () {
      ...
      expect(metadata.links.length, kNyctisMetadataMaxLinks);
    ...
    test('keeps a logo b2 only when it is a real digest', () {
      final good = NyctisAssetMetadata.parse(
        jsonEncode({
          'schema': 'nyctis-asset-metadata/1',
          'logo': {
            'uri': 'https://example.invalid/logo.png',
            'b2': nyctisB2Digest(const [1, 2, 3]),
          },
        }),
      );
      final bad = NyctisAssetMetadata.parse(
        jsonEncode({
          'schema': 'nyctis-asset-metadata/1',
  ...
  group('nyctisUriOrigin', () {
    test('drops a default port and keeps a custom one', () {
      expect(
        nyctisUriOrigin(Uri.parse('https://example.invalid/a/b')),
        'example.invalid',
      );
      expect(
        nyctisUriOrigin(Uri.parse('https://example.invalid:8443/a')),
        'example.invalid:8443',
      );
    });
  });

  group('readNyctisMetadataUri', () {
    NyctisPointerRejection? reject(String uri) =>
        readNyctisMetadataUri(uri).rejection;

    test('accepts https', () {
      final pointer = readNyctisMetadataUri(
        'https://example.invalid/ny/gold.json',
      ).pointer!;
    ...
    test('rejects http', () {
      expect(
        reject('http://example.invalid/ny/gold.json'),
    ...
    test('rejects a uri over the wire limit or outside US-ASCII', () {
      ...
      final long = 'https://example.invalid/${'a' * (kNyctisUriMaxBytes + 1)}';
      expect(long.length, greaterThan(kNyctisUriMaxBytes));
      expect(reject(long), NyctisPointerRejection.malformed);
      expect(
    ...
    test('treats an empty uri as absent, not as a fault', () {
      expect(reject(''), NyctisPointerRejection.absent);
      expect(
        readNyctisMetadataUri(null).rejection,
        NyctisPointerRejection.absent,
      );
    });

    test('reads a #b2= fragment and strips it from the request', () {
      final digest = nyctisB2Digest(utf8.encode('{}'));
      final pointer = readNyctisMetadataUri(
  ...
  group('nyctisB2Digest', () {
    test('matches the BLAKE2b-256 test vector for "abc"', () {
      // bddd813c634239723171ef3fee98579b94964e3bb1cb3e427262c8c068d52319
      final digest = nyctisB2Digest(utf8.encode('abc'));
      ...
      expect(nyctisB2DigestMatches(digest, utf8.encode('abc')), isTrue);
      expect(nyctisB2DigestMatches(digest, utf8.encode('abd')), isFalse);
    });
  });
}
