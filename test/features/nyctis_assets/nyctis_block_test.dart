...
import 'package:zcash_wallet/src/features/nyctis_assets/models/nyctis_block.dart';
...
void main() {
  ...
  NyctisIndexerClient clientFor(FakeNyctisTorBridge bridge) {
  ...
  group('NyctisBlock', () {
    test('reads the block time as a local DateTime', () {
      final block = NyctisBlock.fromJson({
        'height': 7164,
        'hash': 'ab',
        'time': 1789989100,
        'canonical': true,
    ...
    test('a block with no time has no mined-at rather than the epoch', () {
      final block = NyctisBlock.fromJson({'height': 7164, 'hash': 'ab'});
  ...
  group('fetchBlock', () {
    test('asks the indexer for one height and decodes its time', () async {
      final bridge = FakeNyctisTorBridge([nyctisJsonResponse(_blockJson)]);
