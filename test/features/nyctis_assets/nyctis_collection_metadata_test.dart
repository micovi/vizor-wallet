...
NyctisCollectionMetadata _parse(Map<String, Object?> json) =>
    NyctisCollectionMetadata.fromJson(json);
...
void main() {
  ...
  group('section 3.2 — {index} comes from the chain', () {
    ...
    test('substitutes nothing the specification has not defined', () {
      ...
      expect(member.imageRejection, NyctisItemImageRejection.unresolvedToken);
    });
    ...
    test('refuses a substituted image that is not https', () {
      ...
      final member = collection.memberAt(7)!;
      expect(member.image, isNull);
      expect(member.imageRejection, NyctisItemImageRejection.notHttps);
    });
  ...
  group('the document itself', () {
    test('rejects a schema major it does not implement', () {
      expect(
        () => _parse(_document(extra: {'schema': 'nyctis-collection-'
            'metadata/2'})),
    ...
    test('treats a malformed schema as an unrecognized major', () {
      ...
      expect(nyctisSchemaMajor('nyctis-collection-metadata/01',
          kNyctisCollectionSchemaFamily), isNull);
      expect(nyctisSchemaMajor('nyctis-collection-metadata/1',
          kNyctisCollectionSchemaFamily), 1);
    });
  ...
  group('fetching a collection member', () {
    ...
    FakeNyctisTransport transport({
      ...
      Uint8List? image,
      Map<Uri, NyctisHttpReply> extra = const {},
    }) => FakeNyctisTransport({
    ...
    test('a digest mismatch discards the image without retrying', () async {
      ...
      final data = NyctisArtworkData.fromOutcome(outcome);
      expect(data.status, NyctisArtworkStatus.refused);
    ...
    test('a missing digest renders as unpinned, not as a failure', () async {
      ...
      final data = NyctisArtworkData.fromOutcome(outcome);
      expect(data.status, NyctisArtworkStatus.unpinned);
    ...
    test("a member's image over 256 KiB is refused", () async {
      final fake = transport(
        document: _document(),
        image: Uint8List(kNyctisLogoMaxBytes + 1),
      );
      final fetcher = NyctisAssetMetadataFetcher(transport: fake);
    ...
    test('an http: collection uri is refused before any request', () async {
      final fake = FakeNyctisTransport(const {});
      final fetcher = NyctisAssetMetadataFetcher(transport: fake);
    ...
    test('a pinned document reports its pin to the UI', () async {
      ...
      final fake = FakeNyctisTransport({
        ...
        imageUri(1): NyctisHttpReply(statusCode: 200, body: kOnePixelPng),
      });
      final fetcher = NyctisAssetMetadataFetcher(transport: fake);

      final outcome = await fetcher.fetchArtwork(
        assetId: assetId,
        uri: '$documentUri#b2=${nyctisB2Digest(bytes)}',
        index: 1,
      );

      final data = NyctisArtworkData.fromOutcome(outcome);
      expect(data.status, NyctisArtworkStatus.verified);
  ...
  group('an asset document is still an asset document', () {
    test('a uri pointing at one is parsed as one, index or no index', () async {
      final assetDocument = Uri.parse('https://example.invalid/ny/gold.json');
      final logo = Uri.parse('https://example.invalid/ny/gold.png');
      final fake = FakeNyctisTransport({
        assetDocument: NyctisHttpReply(
          statusCode: 200,
        ...
        logo: NyctisHttpReply(statusCode: 200, body: kOnePixelPng),
      });
      final fetcher = NyctisAssetMetadataFetcher(transport: fake);
  ...
  group("the collection's own logo (section 3.5)", () {
    ...
    test('is parsed by the section 4.2 parser, not a second one', () {
      ...
      expect(parsed.logo, isA<NyctisAssetLogoRef>());
    ...
    test('is not pinned by digests, and does not consume an entry', () {
      ...
      final parsed = _parse(
        _document(
          digests: [nyctisB2Digest(kOnePixelPng)],
          extra: {
            'logo': {'uri': logoUri.toString()},
          },
        ),
      ...
      expect(parsed.digestAt(0), nyctisB2Digest(kOnePixelPng));
  ...
  group("fetching the collection's logo", () {
    ...
    FakeNyctisTransport transportWith(Uint8List logoBytes, {String? b2}) =>
        FakeNyctisTransport({
          documentUri: NyctisHttpReply(
            statusCode: 200,
            body: _bytes(
          ...
          for (var i = 0; i < 10; i++)
            Uri.parse('https://example.invalid/pon/$i.png'):
                NyctisHttpReply(statusCode: 200, body: kOnePixelPng),
          logoUri: NyctisHttpReply(statusCode: 200, body: logoBytes),
        });
    ...
    test('once per document, however many members ask for it', () async {
      ...
      final fake = transportWith(kOtherPng, b2: nyctisB2Digest(kOtherPng));
      final fetcher = NyctisAssetMetadataFetcher(transport: fake);
    ...
    test('a document with no logo asks no host for one', () async {
      final fake = FakeNyctisTransport({
        ...
        Uri.parse('https://example.invalid/pon/0.png'): NyctisHttpReply(
          statusCode: 200,
          body: kOnePixelPng,
        ),
      });
